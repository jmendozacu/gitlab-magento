...
// Update old installs to use the "Item Group Id" directive instead of "Parent Product Id"
...
foreach ($rows as $row) {
    ...
    foreach ($column_map as $k => $map) {
        if ($map['attribute'] == 'rw_gbase_directive_parent_id') {
            $column_map[$k]['attribute'] = 'rw_gbase_directive_item_group_id';
            $change = true;
            break;
        }
    }
