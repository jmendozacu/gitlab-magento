...
if (count($rows)) {
    ...
    foreach ($feeds as $store_id => $conf) {
        if ($store_id > 0 && count($conf) && $stores[$store_id]['name'] != 'Admin') {
            ...
            $installer->getConnection()->insert($this->getTable('rocketshoppingfeeds/feed'), array(
                'store_id' => $store_id,
                'name' => 'Default - '. $stores[$store_id]['name'],
                'type' => RocketWeb_ShoppingFeeds_Model_Feed_Type::TYPE_GOOGLE_SHOPPING,
                'status' => $active ? RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_SCHEDULED : RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_DISABLED,
            ...
            foreach ($conf as $path => $value) {
                ...
                $installer->getConnection()->insert($this->getTable('rocketshoppingfeeds/feed_config'), array(
                    'feed_id'   => $feed_id,
                    'path'      => $path,
                    'value'     => $value
                ));
            ...
            $start_at = Mage::getModel('rocketshoppingfeeds/feed_schedule')->getNextStartAt();
            $installer->getConnection()->insert($this->getTable('rocketshoppingfeeds/feed_schedule'), array(
                'feed_id'       => $feed_id,
                'start_at'      => $start_at,
                'processed_at'  => $time,
...
if ($installer->getConnection()->tableColumnExists($this->getTable('rocketshoppingfeeds/process'), 'store_id')
    && !$installer->getConnection()->tableColumnExists($this->getTable('rocketshoppingfeeds/process'), 'feed_id')) {
    $installer->getConnection()->changeColumn($this->getTable('rocketshoppingfeeds/process'), 'store_id', 'feed_id', 'int(11) unsigned NOT NULL', true);
    $installer->getConnection()->addForeignKey($installer->getFkName('rocketshoppingfeeds/process', 'feed_id', 'rocketshoppingfeeds/feed', 'id'),
        $this->getTable('rocketshoppingfeeds/process'), 'feed_id', $installer->getTable('rocketshoppingfeeds/feed'), 'id',
...
foreach ($feeds as $feed) {
    ...
    if ($categoryTree == null) {
        $categories = Mage::helper('rocketshoppingfeeds')->getAllCategories($feed, true, true);
        ...
        foreach($originalCategories as $category) {
            if (isset($category['category'])) {
                $taxonomyCategories[$category['category']] = array(
                    ...
                    'disabled' => false //!in_array($category['category'], $enabledCategories)
                );
            }
        }
    }
    ...
    $sql = Mage::getModel('rocketshoppingfeeds/config')->getCollection()
        ->addFieldToSelect('id')
    ...
    Mage::getModel('rocketshoppingfeeds/config')
        ->load($configId)
        ->addData(array(
            'feed_id' => $feed->getId(),
    ...
    $model = Mage::getModel('rocketshoppingfeeds/config')
        ->load($configId)
        ->addData(array(
            'feed_id' => $feed->getId(),
