...
class RocketWeb_ShoppingFeeds_Adminhtml_RocketfeedController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Initialize action
     *
    ...
    protected function _initAction()
    {
        $this->loadLayout()
            ...
            ->_setActiveMenu('catalog/rocketshoppingfeeds_feed')
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('catalog/rocketweb_shoppingfeeds');
    }
    ...
    protected function _getFeed()
    {
        ...
        $feed = Mage::getModel('rocketshoppingfeeds/feed')->load($feed_id);
        if (!$feed->getId()) {
    ...
    public function listAction()
    {
        ...
        $this->_initAction()
            ->_addBreadcrumb($this->__('Rocket Feeds'), $this->__('Rocket Feeds'));
        if ($helper ->isCronEnabled() && !$helper->cronHasHeartbeat()) {
        ...
        if (!$helper ->isCronEnabled()) {
            $cron_message = '</li><li>' . $helper->__('Magento Cron is disabled - to run the feeds on schedule, please <a href="https://wiki.rocketweb.com/display/RSF/Detaching+feed+generation+from+magento+cron" target="_blank">setup a custom cron</a>');
        }

        $this->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed'))
            ->_addContent($this->getLayout()->createBlock('core/text', 'adminhtml_feed.grid.help')->setText(
                ...
                . sprintf(Mage::helper('rocketshoppingfeeds')->__('Please use our %s to help you set up feeds.'), '<a target="_blank" href="https://wiki.rocketweb.com/display/RSF" title="Quick Start">User Guide</a>'). '</li><li>'
                . Mage::helper('rocketshoppingfeeds')->__('Once a feed is pending processing, it will start with a delay corresponding to your cron.php frequency.'). ' <br>'
                . sprintf(Mage::helper('rocketshoppingfeeds')->__('To enable microdata on your product pages, Configure a Google feed and enable microdata on it. You need to enable Microdata in one feed per store view.'))
                . $cron_message
                . '</li></ul></li></ul>'
            ));
        ...
        $cell = $this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_grid_schedulegridcell');
    ...
    {
        $this->loadLayout();
        $grid = $this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed');
    ...
    public function newAction()
    {
        ...
        $this->_initAction()
            ...
            ->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_new')
    ...
    public function editAction()
    {
        ...
        /** @var RocketWeb_ShoppingFeeds_Model_Feed $model */
        $model = Mage::getModel('rocketshoppingfeeds/feed')
            ->setType($this->getRequest()->getParam('type', RocketWeb_ShoppingFeeds_Model_Feed_Type::TYPE_GENERIC))
            ->setName($this->getRequest()->getParam('name', ''))
            ->load($feed_id);

        Mage::register('rocketshoppingfeeds_feed', $model);
        Mage::register('rocketshoppingfeeds_feed_layout', $this->getLayout());
        
        Mage::getSingleton('rocketshoppingfeeds/feed_taxonomy')->getProvider($model)->prepareTaxonomyFiles();

        $title = $model->getId() ? sprintf($this->__('Edit %s'), $model->getName()) : sprintf($this->__('New %s Feed'), ucfirst($this->getRequest()->getParam('type', RocketWeb_ShoppingFeeds_Model_Feed_Type::TYPE_GENERIC)));
        $this->_title($title);

        $this->_initAction()
            ->_addBreadcrumb($title, $title)
            ->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_edit')->setData('action', $this->getUrl('*/*/save')))
            ->_addLeft($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_edit_tabs'));
    ...
    public function saveAction()
    {
        if ($postData = $this->getRequest()->getPost()) {
            try {
                ...
                $feed = Mage::getModel('rocketshoppingfeeds/feed')
                    ->setType($this->getRequest()->getParam('type', RocketWeb_ShoppingFeeds_Model_Feed_Type::TYPE_GENERIC))
                ...
                if (!array_key_exists('id', $postData)) {
                    // Since a saved was already tried and saved, clear the generated ID, to retry from scratch
                    $feed->unsetData('id');
                }
                $feed->setData('ignore_warnings', 1)->save();
            }
    ...
    public function deleteAction() {
        ...
        try {
            Mage::getModel('rocketshoppingfeeds/feed')->load($feedId)->delete();
    ...
    public function generateAction()
    {
        try {
            ...
            Mage::getModel('rocketshoppingfeeds/queue')->send($feed, 'manual');
            $feed->saveStatus(RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_PENDING);
            if ($feed->getSchedule()->getBatchMode()) {
                @unlink(Mage::helper('rocketshoppingfeeds')->getGenerator($feed)->getBatchLockPath());
            }
    ...
    public function generateAllAction()
    {
        try {
            $cnt = 0;
            $feeds = Mage::getModel('rocketshoppingfeeds/feed')->getCollection();
        ...
        try {
            ...
            $this->_addContent(
                $this->getLayout()->createBlock('core/template', 'rw_google_feed_log',
                    array('feed' => $feed, 'generator' => $generator, 'template' => 'rocketshoppingfeeds/system/log.phtml',
    ...
    {
        $this->_massStatus(RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_SCHEDULED);
    }
    ...
    {
        $this->_massStatus(RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_DISABLED);
        // remove the queue leftovers
        $feedIds = (array)$this->getRequest()->getParam('ids');
        $queue = Mage::getModel('rocketshoppingfeeds/queue')->getCollection()
    ...
    public function massCloneAction()
    {
        ...
        if(!is_array($feedIds)) {
            ...
            foreach ($feedIds as $feedId) {
                ...
                if (!empty($config)) {
                    $new_feed = Mage::getModel('rocketshoppingfeeds/feed')
                        ->setData($feed->getData())
                        ->setId(null)
                        ->setName($feed->getName() . ' (cloned)')
                        ->setConfig($feed->getConfig());

                    if ($new_feed->getStatus()->getCode() != RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_DISABLED) {
                        $new_feed->setStatus(RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_SCHEDULED);
                    }
                    $scheduleModel = Mage::getModel('rocketshoppingfeeds/feed_schedule');
    ...
    public function exportAction()
    {
        ...
        if ($feed_id) {
            ...
            /** @var RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Grid $grid */
            $grid = $this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_grid');
            $grid->getXml();
            ...
            foreach ($grid->getCollection() as $feed) {
                 ...
                 */
                $feed = Mage::getModel('rocketshoppingfeeds/feed')->load($feed->getId());
                $xml .= $feed->toXml();
            }
        }
        $xml .= '</items>';
    ...
    {
        $model = Mage::getModel('rocketshoppingfeeds/feed')->load(0)
            ->setName('Sample Feed');
    ...
    public function importAction()
    {
        ...
        $this->_initAction()
            ...
            ->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_import_edit'))
    ...
    public function startImportAction()
    {
        ...
        if ($data) {
            ...
            $importModel = Mage::getModel('rocketshoppingfeeds/feed_import');
    ...
    public function testAction()
    {
        ...
        try {
            $feed = $this->_getFeed();
            Mage::register('rocketshoppingfeeds_feed', $feed);

            $this->_addBreadcrumb($this->__('Test Feed'), $this->__('Test Feed'))
                ->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_test', 'rw_google_feed_test')
                    ->setData('action', $this->getUrl('*/*/test')));

            if ($postData = $this->getRequest()->getPost()) {
                $this->_addContent($this->getLayout()->createBlock('rocketshoppingfeeds/adminhtml_feed_test_results', 'rw_google_feed_test_results', $postData));
            }
    ...
    public function autocompleteAction()
    {
        ...
        if ($feed) {
            ...
            /** @var RocketWeb_ShoppingFeeds_Model_Provider_Generic_Taxonomy $taxonomyModel */
            $taxonomyModel = Mage::getSingleton('rocketshoppingfeeds/feed_taxonomy')->getProvider($feed);
            $results = $taxonomyModel->prepareTaxonomyFiles()->getTaxonomyList($partial);
        }
    ...
    public function testftpAction()
    {
        ...
        if ($account->getPassword() == RocketWeb_ShoppingFeeds_Model_Feed_Ftp::OBSCURED_VALUE) {
            $ftpAccount = Mage::getModel('rocketshoppingfeeds/feed_ftp')->load($account->getId());
            $account->setPassword($ftpAccount->getPassword());
            $encrypted = true;
        }

        $account->setTimeout(10);
        $result = Mage::helper('rocketshoppingfeeds')->ftpUpload($account, $encrypted);
