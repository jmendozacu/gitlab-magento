 ...
 * @author    RocketWeb
 */
class RocketWeb_ShoppingFeeds_Model_Adapter_Configurable_Associated
    extends RocketWeb_ShoppingFeeds_Model_Adapter_Abstract_Associated
{
    ...
    public function getPrices()
    {
        ...
        $op_excl_tax = $op_incl_tax = 0;
        $sop_excl_tax = $sop_incl_tax = 0;
                ...
                if (is_array($res['values']) && count($res['values'])) {
                    ...
                    foreach ($res['values'] as $value) {
                        if (isset($value['value_index']) && $this->getProduct()->getData($res['attribute_code']) == $value['value_index']) {
                            ...
                            $option_price = $this->convertPrice($option_price);
                            $option_sale_price = $this->convertPrice($option_sale_price);
                            /**
                            ...
                            $op_incl_tax += $taxHelper->getPrice($product, $option_price, true);

                            $sop_excl_tax += $taxHelper->getPrice($product, $option_sale_price);
                            $sop_incl_tax += $taxHelper->getPrice($product, $option_sale_price, true);
                            break;
                        }
                    }
                }
        ...
        $prices['p_incl_tax'] = $taxHelper->getPrice($product, $convertedPrice, true, null, null, null, null, null, false) + $op_incl_tax;
        ...
        $prices['sp_excl_tax'] = $taxHelper->getPrice($product, $convertedFinalPrice) + $sop_excl_tax;
        $prices['sp_incl_tax'] = $taxHelper->getPrice($product, $convertedFinalPrice, true) + $sop_incl_tax;
