 ...
 * @author    RocketWeb
 */
class RocketWeb_ShoppingFeeds_Model_Map_Google_Product_Bundle
    extends RocketWeb_ShoppingFeeds_Model_Map_Abstract
{

    /**
     * Get the identifier value from the most expensive associated item
     *
     * @param array $params
     * @return string
     */
    public function mapDirectiveIdentifier($params = array())
    {
        ...
        if (empty($val)) {
            $adapter = Mage::helper('rocketshoppingfeeds')->sortAssocsByPrice($this->getAdapter()->getAssocAdapters(), 'max');
            if (!is_null($adapter)) {
