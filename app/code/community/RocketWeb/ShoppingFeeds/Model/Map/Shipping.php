 ...
 * @author    RocketWeb
 */
class RocketWeb_ShoppingFeeds_Model_Map_Shipping extends Varien_Object
{
    ...
    /**
     * @var RocketWeb_ShoppingFeeds_Helper_Tax
     */
     ...
     * @return RocketWeb_ShoppingFeeds_Model_Map_Generic_Product_Shipping
     */
    public function setItem($product, $parentProduct = null)
    {
        ...
        $this->_item->setData('qty', 1);
        $this->_item->setData('weight', $product->getWeight());
     ...
     */
    protected function setRequest($item, $limitCarrier, $address, $country_id = null, $region_id = null, $postcode = null)
    {
        $request = Mage::getModel('shipping/rate_request');
        $item->setAddress($address);
        $item->setProductId($item->getProduct()->getId());

        $map_prices = $this->getMapProduct()->getAdapter()->getPrices();
        ...
        $weight = $this->getMapProduct()->getAdapter()->mapColumn($ship_column);
        if ($unit != "") {
        ...
        // City and postcode usually not provided but is required by Webshops_Matrixrate, so we fill in the origin ones to get the lowest rate.
        $city = null;
        $originCountryId = Mage::getStoreConfig(Mage_Shipping_Model_Shipping::XML_PATH_STORE_COUNTRY_ID, Mage::app()->getStore($this->getStoreId()));
        $originRegionId = Mage::getStoreConfig(Mage_Shipping_Model_Shipping::XML_PATH_STORE_REGION_ID, Mage::app()->getStore($this->getStoreId()));
        if ($region_id == $originRegionId && $country_id == $originCountryId) {
            if (is_null($postcode)) {
                $postcode = Mage::getStoreConfig(Mage_Shipping_Model_Shipping::XML_PATH_STORE_ZIP, Mage::app()->getStore($this->getStoreId()));
            }
            $city = Mage::getStoreConfig(Mage_Shipping_Model_Shipping::XML_PATH_STORE_CITY, Mage::app()->getStore($this->getStoreId()));
        }
    ...
    public function getFormatedValue()
    {
        ...
        /** @var RocketWeb_ShoppingFeeds_Model_Adapter_Abstract $mapProduct */
        $mapProduct = $this->getMapProduct()->getAdapter();
                            ...
                            foreach ($carriers as $carrier) {
                                if (is_array($carrier) && isset($carrier['methods']) && is_array($carrier['methods'])) {
                                    foreach ($carrier['methods'] as $method) {
                                        $price = $mapProduct->convertPrice($this->getPriceTax($method['price'], $country_id)) . ' '. $mapProduct->getData('store_currency_code');
                            ...
                            foreach ($carriers as $carrier) {
                                if (is_array($carrier) && isset($carrier['methods']) && is_array($carrier['methods'])) {
                                    foreach ($carrier['methods'] as $method_code => $method) {
                                        ...
                                        $address->setQuote($quote);
                                        $address->setCountryId($country_id);
                                        $price = $mapProduct->convertPrice($this->getPriceTax($method['price'], $country_id, $region_id)) . ' '. $mapProduct->getData('store_currency_code');
    ...
    /**
     * @return Mage_Core_Helper_Abstract|RocketWeb_ShoppingFeeds_Helper_Tax
     */
    protected function _getTaxHelper()
    {
        if (is_null($this->_taxHelper)) {
            $this->_taxHelper = Mage::helper('rocketshoppingfeeds/tax');
        }
