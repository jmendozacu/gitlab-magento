 ...
 * @author     RocketWeb
 */
class RocketWeb_ShoppingFeeds_Model_Feed_Import extends Mage_ImportExport_Model_Import
{
    /**
    ...
    {
        $this->addLogComment(Mage::helper('rocketshoppingfeeds')->__('Begin data validation'));
        ...
        if($loadedXml === FALSE) {
            Mage::throwException(Mage::helper('rocketshoppingfeeds')->__('XML data is not valid. Please check and resubmit the file.'));
        } else {
        ...
        if (!empty($sourceFile) && file_exists($sourceFile)) {
            ...
            if($xml instanceof SimpleXMLElement) {
                ...
                foreach($xmlItems as $item) {
                    ...
                    foreach ((array)$item as $itemFieldName => $itemField) {
                        ...
                        if (!in_array($itemFieldName, array('id', 'config', 'schedule'))) {
                            ...
                            if ($itemFieldName == 'status') {
                                $feed->setData($itemFieldName, RocketWeb_ShoppingFeeds_Model_Feed_Status::STATUS_SCHEDULED);
                            }
                        }
                        // Fill in config data
                    ...
                    // we mark the instance as new because we need to make this distinction in Feed::_afterSave()
                    $feed->isObjectNew(true);
                    if ($feed->save()) {
                        $count ++;
                        foreach ($ftps as $ftp) {
                            $ftpModel = Mage::getModel('rocketshoppingfeeds/feed_ftp')
                                ->setData($ftp)
    ...
    public function uploadSource()
    {
        ...
        if (!$extension) {
            unlink($uploadedFile);
            Mage::throwException(Mage::helper('rocketshoppingfeeds')->__('Uploaded file has no extension'));
        }
        ...
        if(strtolower($uploadedFile) != strtolower($sourceFile)) {
            ...
            if (!@rename($uploadedFile, $sourceFile)) {
                Mage::throwException(Mage::helper('rocketshoppingfeeds')->__('Source file moving failed'));
            }
        }
        return $sourceFile;
    }
