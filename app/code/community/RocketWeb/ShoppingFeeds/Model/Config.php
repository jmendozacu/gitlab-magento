 ...
 */
class RocketWeb_ShoppingFeeds_Model_Config extends Mage_Core_Model_Abstract
{
    /*
     * We keep using feedconfigdata naming for time saving purposes and legacy overall.
     */
    protected function _construct() {
        $this->_init('rocketshoppingfeeds/config');
    }
    ...
    public function _beforeSave()
    {
        switch($this->getPath()) {
            case 'columns_map_product_columns':
                $value = Mage::getModel('rocketshoppingfeeds/system_config_backend_serialized_mapproductcolumns')
                    ->setData($this->getData())
                    ->filterValueBeforeSave($this->getValue());
                $this->setValue($value);
                break;
            case 'google_promotions':
                if (is_array($this->getValue())) {
                    $model = Mage::getModel('rocketshoppingfeeds/provider_google_promotions');
                    $value = $model->beforeSave($this->getValue());
                    $this->setValue($value);
                }
            case 'categories_provider_taxonomy_by_category':
                $value = Mage::getModel('rocketshoppingfeeds/system_config_backend_serialized_json')
                    ->filterValueBeforeSave($this->getValue());
                $this->setValue($value);
                break;
            ...
            case 'filters_find_and_replace':
                $value = Mage::getModel('rocketshoppingfeeds/system_config_backend_serialized')
                    ->filterValueBeforeSave($this->getValue());
                $this->setValue($value);
                break;
            ...
            case 'columns_stock_attribute_code':
                $value = Mage::getModel('rocketshoppingfeeds/system_config_backend_attributecode')
                    ->filterValueBeforeSave($this->getValue());
                $this->setValue($value);
                break;
        }
