...
class RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_General
    extends RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Abstract
{
    protected function _prepareForm()
    {
        ...
        $microdata_type_note = '';
        if ($feed->getType() !== RocketWeb_ShoppingFeeds_Model_Feed_Type::TYPE_GOOGLE_SHOPPING) {
            $microdata_type_note =
                '<span style="font-weight:bold; color: red;">'
                    . $helper->__('To turn on microdata, the feed type should be Google Shopping.')
                .'</span><br>'
                . $helper->__('Do not use Microdata with other feed types unless you know what you are doing.');
        }

        $this->getFieldset()->addField('use_for_microdata', 'select', array(
            'name'      => 'use_for_microdata',
            'label'     => $helper->__('Use for microdata'),
            'required'  => false,
            'values'    => Mage::getModel('adminhtml/system_config_source_yesno')->toOptionArray(),
            'after_element_html' => '<p class="note" style="width:450px;">'
                . $helper->__('Set this to Yes to enable Microdata for this store view, with settings from this feed.')
                . '<br>'. $helper->__('<b>WARNING</b>: You need to submit this feed to Google Shopping, otherwise Microdata will not work properly and you will have issues in Merchant Center.')
                . '<br>' . $microdata_type_note . '</p>'
        ));

        $fieldset->addType('advanced_select', Mage::getConfig()->getBlockClassName('rocketshoppingfeeds/adminhtml_form_field_select'));
        $fieldset->addField('general_currency', 'advanced_select', array(
            'name' => 'config[general_currency]',
            'label' => $helper->__('Feed Currency'),
            'required' => true,
            'values' => Mage::getSingleton('rocketshoppingfeeds/source_currency')->toOptionArray(),
            'after_element_html' => '<p class="note" style="width:450px;">'. $helper->__('This lists only allowed currencies on the store view.') . '<br>'
                . $helper->__('<b>WARNING</b>: Changing to a currency which is not displayed on frontend can lead to feed being rejected by the provider!') . '</p>'
        ));
        ...
        $field['stock_attribute_code'] = $this->addField('general_stock_attribute_code', 'select', array(
            ...
            'required'  => true,
            'values'    => Mage::getModel('rocketshoppingfeeds/source_productattributescodes')->toOptionArray(),
        ...
        $this->addField('general_use_image_cache', 'select', array(
            ...
            'required'  => true,
            'values'    => Mage::getModel('adminhtml/system_config_source_yesno')->toOptionArray(),
            'after_element_html' => '<p class="note" style="width:450px;">'. $helper->__('Enabling image cache will cause output of Product Image URL and Product Additional Image URL directives to attempt create url to images using cache instead of direct media/image url.'). '</p>'
        ));
        ...
        $data['use_for_microdata'] = $feed->getData('use_for_microdata');
