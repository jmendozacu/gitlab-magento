 ...
 * @author     RocketWeb
 */
/* Class RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Ftp
 * 
    ...
    extends RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Abstract
{
    ...
    {
        parent::__construct();
        $this->setTemplate('rocketshoppingfeeds/grid/ftp.phtml');
        $feed = Mage::registry('rocketshoppingfeeds_feed');
        if (!$feed || ($feed && !$feed->getId())) {
            if (!$feed) {
                $feed = Mage::getModel('rocketshoppingfeeds/feed')->load(0);
            }
        }
        $this->setFeed($feed);
        /**
     ...
     *
     * @return RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Schedule
     */
    protected function _prepareLayout()
    {
        ...
        $this->setChild('add_button',
            $this->getLayout()->createBlock('adminhtml/widget_button')
                ->setData(array(
                    'label' => Mage::helper('rocketshoppingfeeds')->__('Add Account'),
                    'class' => 'add',
    ...
    public function getTestButtonHtml($id = false, $hidden = false)
    {
        ...
        return $this->getLayout()->createBlock('adminhtml/widget_button')
            ->setData(
                array(
                    'label'     => Mage::helper('rocketshoppingfeeds')->__('Validate'),
                    'class'     => 'save',
    ...
    public function getAccounts()
    {
        if (!$this->hasData('accounts')) {
            if ($this->getFeed()->getId()) {
                $accounts = Mage::getResourceModel('rocketshoppingfeeds/feed_ftp_collection')
    ...
    {
        if ($value) {
            $value = RocketWeb_ShoppingFeeds_Model_Feed_Ftp::OBSCURED_VALUE;
        }
     ...
     * @param string $field
     * @param string|boolean $id
     * @param RocketWeb_ShoppingFeeds_Model_Feed_Ftp $account
     * @return string
     */
     ...
     * Retrieve path value
     * 
     * @param RocketWeb_ShoppingFeeds_Model_Feed_Ftp|boolean $account
     * @return string
     */
    ...
    public function getModeHtml($id = false, $value = false)
    {
        if (!$this->hasData('mode_select')) {
            ...
            foreach (Mage::getModel('rocketshoppingfeeds/source_ftp')->toArray() as $value => $label) { 
     ...
     * @return string
     */
    public function getGzipHtml($id = false, $value = false)
    {
        if (!$this->hasData('gzip_select')) {
            $select = $this->getLayout()->createBlock('core/html_select');
            foreach (Mage::getModel('adminhtml/system_config_source_yesno')->toOptionArray() as $row) {
                $select->addOption($row['value'], $row['label']);
            }
            $this->setData('gzip_select', $select);
        } else {
            $select = $this->getData('gzip_select');
        }
        $id = $id ? $id : '{{id}}';
        $select->setId("ftp_gzip_$id");
        $select->setName("ftp[$id][gzip]");
        $select->setValue($value);
        return $select->toHtml();
    }
     ...
     * @return string
     */
    public function getGzipValue($account = false)
    {
        if ($account != false && is_object($account)) {
            return $account->getGzip();
        }
        return '';
    }
}
