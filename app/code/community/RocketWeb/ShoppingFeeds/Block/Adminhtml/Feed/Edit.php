...
class RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    /**
     * Init class
     */
    public function __construct()
    {
        ...
        $id = Mage::registry('rocketshoppingfeeds_feed')->getId();
        if ($id) {
    ...
    public function getHeaderText()
    {
        ...
        if ($feed->getId()) {
            $str = Mage::helper('rocketshoppingfeeds')->__('Edit %s');
        }
        else {
            $str = Mage::helper('rocketshoppingfeeds')->__('New %s');
        }
        return sprintf($str, RocketWeb_ShoppingFeeds_Model_Feed_Type::getLabel($feed->getType()));
    }
