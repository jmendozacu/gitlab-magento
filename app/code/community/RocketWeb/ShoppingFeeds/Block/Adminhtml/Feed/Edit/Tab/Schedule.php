...
class RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Schedule
    extends RocketWeb_ShoppingFeeds_Block_Adminhtml_Feed_Edit_Tab_Abstract
{
    ...
    {
        parent::__construct();
        $this->setTemplate('rocketshoppingfeeds/grid/schedule.phtml');
        ...
        $this->_defaultValues['start_at'] = Mage::getModel('rocketshoppingfeeds/feed_schedule')->getNextStartAt();
    ...
    public function getSchedules()
    {
        if (!$this->_schedules) {
            if ($this->getFeed()->getId()) {
                $this->_schedules = Mage::getResourceModel('rocketshoppingfeeds/feed_schedule_collection')
    ...
    public function getStartAtHtml($id = false, $value = false)
    {
        if (!$this->_startAt) {
            ...
            for($ii = 0; $ii < 24; $ii++) {
                $label = Mage::getSingleton('rocketshoppingfeeds/feed_schedule')->getTimeFormatted($ii);
