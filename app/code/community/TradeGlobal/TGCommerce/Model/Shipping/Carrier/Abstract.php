 ...
 * but we don't trust it not to change, and we want to be able to mod it if we need to.
 *
 * @author      Paul Snell
 * @category    TradeGobal
 ...
 */
abstract class TradeGlobal_TGCommerce_Model_Shipping_Carrier_Abstract extends Mage_Shipping_Model_Carrier_Abstract
{
    ...
    public function proccessAdditionalValidation(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        foreach ($this->getAllItems($request) as $item) {
            if ($item->getProduct() && $item->getProduct()->getId()) {
                ...
                if ($doValidation && $maxAllowedWeight != 0 && $weight > $maxAllowedWeight) {
