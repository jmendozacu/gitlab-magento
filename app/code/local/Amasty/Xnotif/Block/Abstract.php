...
class Amasty_Xnotif_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    private function _loadCollection()
    {
        ...
        $select->joinInner(
            ...
            array( 'add_date', $entityIdName,'parent_id')
        )
            ->where('s.status=0')
    ...
    {
        $url = $_order->getParentId()?
            $this->getProduct($_order->getParentId())->getProductUrl()
            : $this->getProduct($_order->getEntityId())->getProductUrl();
