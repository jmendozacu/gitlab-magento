...
class Amasty_Promo_Block_Adminhtml_Promo_Quote_Edit extends Mage_Adminhtml_Block_Promo_Quote_Edit
{
    ...
    public function __construct()
    {
        ...
        $eachnLabelForDiscountStep       = $this->__('Each N-th');
        $defaultLabelForDiscountStep     = $this->__('Discount Qty Step (Buy X)');
        $this->_formScripts[] = "
            ...
            function ampromo_hide() {
                ...
                if ('ampromo_eachn' == $('rule_simple_action').value){
                    $('rule_simple_free_shipping').up().up().hide();

                    if (s) s.up().up().hide();
                    $('rule_promo_sku').up().up().show();
                    $('rule_ampromo_type').up().up().show();
                    $$(\"label[for='rule_discount_step']\")[0].update(\"" . $eachnLabelForDiscountStep . " \");
                } else {
                    $$(\"label[for='rule_discount_step']\")[0].update(\"" . $defaultLabelForDiscountStep . " \");
                }
