...
class Amasty_Rules_Model_Observer
{
    ...
    * */
     ...
     * @var bool
     */
    protected $rulePrevStopFurtherChange = false;
    ...
    public function handleValidation($observer)
    {
        ...
        if (!$this->stopFurtherRules && (!$this->checkStopPreviousRule() || $this->rulePrevStopFurtherChange)) {
            $this->setStopRuleAndItem($observer->getRule(), $observer->getItem());
            $promotions->process($observer);

            /**uncomment if need use default logic of "Store Further..." for default rules Magento **/
//            if (!$promotions->isSpecialPromotions && $this->stopFurtherRules) {
//                $observer->getRule()->setStopRulesProcessing(false);
//                $this->stopFurtherRules = false;
//                $this->rulePrevStopFurtherChange = true;
//            }
        }
    ...
    protected function checkStopPreviousRule()
    {
        ...
        if ($this->rules->getSize()) {
            foreach ($this->rules as $item) {
                if (Mage::helper('amrules')->isStopRuleProcessing($item) && $item->validate($currentCart)) {
                    $result = true;
                }
            }
        }
