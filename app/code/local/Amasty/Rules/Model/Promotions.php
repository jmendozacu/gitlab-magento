...
class Amasty_Rules_Model_Promotions
{
    ...
    public function process($observer)
    {
        ...
        } elseif (in_array($rule->getSimpleAction(),
                array('ampromo_items', 'ampromo_cart', 'ampromo_spent', 'ampromo_product')) && $item->getIsPromo()) {
    ...
    protected function _limitMaxDiscount($r, $rule, $itemId, $quote)
    {
        ...
        if (isset($this->_ruleDiscount[$ruleId])) {
            $this->_ruleDiscount[$ruleId] += $r[$itemId]['base_discount'];
        } else {
            $this->_ruleDiscount[$ruleId] = $r[$itemId]['base_discount'];
        }

        if ($this->_ruleDiscount[$ruleId] > $maxDiscount) {
            $canAdd = $maxDiscount - ($this->_ruleDiscount[$ruleId] - $r[$itemId]['base_discount']);
            if ($canAdd > 0) {
                $r[$itemId]['base_discount'] = $canAdd;
                $r[$itemId]['discount'] = max(0, $quote->getStore()->convertPrice($r[$itemId]['base_discount']));
            } else {
                $r[$itemId]['base_discount'] = 0;
                $r[$itemId]['discount'] = max(0, $quote->getStore()->convertPrice($r[$itemId]['base_discount']));
            }
        }
