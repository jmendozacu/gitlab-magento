    ...
    implements Mage_Index_Model_Resource_Helper_Lock_Interface
{
    ...
    {
        return (bool) $this->_getWriteAdapter()->query("SELECT GET_LOCK(?, ?);", array(sha1($name), self::LOCK_GET_TIMEOUT))
            ->fetchColumn();
    }
    ...
    {
        return (bool) $this->_getWriteAdapter()->query("SELECT RELEASE_LOCK(?);", array(sha1($name)))->fetchColumn();
    }
    ...
    {
        return (bool) $this->_getWriteAdapter()->query("SELECT IS_USED_LOCK(?);", array(sha1($name)))->fetchColumn();
    }
