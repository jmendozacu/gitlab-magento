 ...
 */
class Ebizmarts_SagePaymentsPro_TokenController extends Mage_Core_Controller_Front_Action
{
    public function indexAction()
    {
        ...
        if ($navigationBlock = $this->getLayout()->getBlock('customer_account_navigation')) {
            $navigationBlock->setActive('sgusa/token');
        }
        ...
        $this->getLayout()->getBlock('head')->setTitle(
            Mage::helper('ebizmarts_sagepaymentspro')->__('SagePayments - Saved Credit Cards')
        );
        $this->renderLayout();
    }
    ...
    public function registerAction()
    {
        ...
        if ($this->getRequest()->isPost()) { #DIRECT POST
            ...
            if (empty($rs)) {
                ...
                $rs['StatusDetail'] = Mage::helper('ebizmarts_sagepaymentspro')->
                    __('A server to server communication error ocured, please try again later.');
            }
            ...
            if ($rs['result'] == 'SUCCESS') {
                ...
                $resp ['mark'] = array(
                    ...
                    'delurl' => Mage::getUrl('sgusa/token/delete', array('card'=>$save->getId()))
                );
            } else {
                $rs = array_change_key_case($rs);
                $resp = $rs;
        ...
        $url = '';
        if (false ===
            Mage::getModel('ebizmarts_sagepaymentspro/tokencard')->customerCanAddCard()) {
