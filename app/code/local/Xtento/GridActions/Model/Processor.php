 ...
 * ID:            %!uniqueid!%
 * Packaged:      %!packaged!%
 * Last Modified: 2016-09-02T16:26:53+02:00
        ...
        foreach ($orderIds as $orderId) {
            try {
                ...
                if ($orderId === "undefined") {
                    // Some customized grids add a first entry with "id" "undefined" (string) - should be skipped
                    continue;
                }
                ...
                if ($doUncancel && $order->getStatus() == Mage_Sales_Model_Order::STATE_CANCELED) {
                            ...
                            foreach ($tracksAndCarriers[$orderId] as $trackData) {
                                ...
                                if (!$trackAlreadyAdded) {
                                    if (!empty($trackingNumber)) {
                                        ...
                                        $lastShipment->sendEmail($doNotify, '');
                                    }
                                }
                            }
                            unset($lastShipment);
