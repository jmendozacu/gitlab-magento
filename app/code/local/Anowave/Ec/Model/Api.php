...
class Anowave_Ec_Model_Api
{
	...
	const TRIGGER_ADD_WISHLIST								= 'Event Equals Add To Wishlist';
	const TRIGGER_ADD_COMPARE								= 'Event Equals Add To Compare';
	...
	const TAG_WISHLIST										= 'EE Add To Wishlist';
	const TAG_COMPARE										= 'EE Add To Compare';
	...
	public function ec_api_variables($account, $container)
	{
		...
		(
			...
			'ee event target' => array
			(
				'name' 		=> 'ee event target',
				'type'		=> self::TAG_MANAGER_VARIABLE_TYPE_DATALAYER_VARIABLE,
				'parameter' => array
				(
					array
	...
	public function ec_api_tags($account, $container)
	{
		...
		(
			...
			self::TAG_WISHLIST => array
			(
				'name' 				=> self::TAG_WISHLIST,
				'firingTriggerId' 	=> array
				(
			...
			self::TAG_COMPARE => array
			(
				'name' 				=> self::TAG_COMPARE,
				'firingTriggerId' 	=> array
				(
		 ...
		 */
		if (isset($_POST['ec_api_ua']))
		{
			$schema[self::TAG_UA] = array
			(
				'name' 				=> self::TAG_UA,
				'firingTriggerId' 	=> array
				(
	...
	{
		return Mage::getStoreConfig('ec/api/google_gtm_ua', $this->getStoreId());
	}
	 ...
	 * return string
	 */
	public function getAdwordsConversionId()
	{
		if ('' === (string) Mage::getStoreConfig('ec/api/google_adwords_conversion_id', $this->getStoreId()))
		{
			return 0;
		}
		
		return Mage::getStoreConfig('ec/api/google_adwords_conversion_id', $this->getStoreId());
	}
	 ...
	 * @return string
	 */
	public function getAdwordsConversionLabel()
	{
		if ('' === (string) Mage::getStoreConfig('ec/api/google_adwords_conversion_label', $this->getStoreId()))
		{
			return 0;
		}
		
		return Mage::getStoreConfig('ec/api/google_adwords_conversion_label', $this->getStoreId());
	}
	
	/**
	 * Get current store (configuraton scope)
	 *
	 * @return number
	 */
	public function getStoreId()
	{
		if (!$this->store)
		{
			...
			{
				$this->store = (int) Mage::getModel('core/store')->load($code)->getId();
			}
			elseif (strlen($code = Mage::getSingleton('adminhtml/config_data')->getWebsite()))
			{
				$this->store = (int) Mage::app()->getWebsite(Mage::getModel('core/website')->load($code)->getId())->getDefaultStore()->getId();
			}
			else
			{
				
				if (null !== $id = Mage::app()->getRequest()->getParam('store'))
				{
					if (is_numeric($id))
					{
						$this->store = (int) $id;
					...
					elseif (is_string($id))
					{
						$this->store = (int) Mage::getModel('core/store')->loadConfig($id)->getId();
					}
					else 
					{
						$this->store = 0;
