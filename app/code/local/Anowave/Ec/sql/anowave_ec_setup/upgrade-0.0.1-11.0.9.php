...
$sql[] = "CREATE TABLE IF NOT EXISTS " . Mage::getConfig()->getTablePrefix() . "anowave_ab_attributes (ab_attribute_id int(6) NOT NULL AUTO_INCREMENT,ab_id int(6) NOT NULL,ab_attribute_code varchar(255) DEFAULT NULL,PRIMARY KEY (ab_attribute_id),UNIQUE KEY ab_id (ab_id,ab_attribute_code),CONSTRAINT anowave_ab_attributes_ibfk_1 FOREIGN KEY (ab_id) REFERENCES " . Mage::getConfig()->getTablePrefix() . "anowave_ab (ab_id) ON DELETE CASCADE ON UPDATE CASCADE) ENGINE=InnoDB DEFAULT CHARSET=utf8";
