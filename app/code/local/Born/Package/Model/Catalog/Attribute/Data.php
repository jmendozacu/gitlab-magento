...
class Born_Package_Model_Catalog_Attribute_Data extends Varien_Object 
{
    ...
    public function getAttributeValue($entityId, $attributeId, $_tableName = null)
    {
        ...
        if (!is_numeric($attributeId) || !is_numeric($entityId)) {
            //Mage::log('Born_Package_Model_Catalog_Attribute_Data::getProductAttributeValue: one of the follow values are not numeric: ' . $attributeId . ', ' . $entityId);
            return;
        }

        $storeIds = array(Mage::app()->getStore()->getStoreId(), '0');

        //Fetches the row from with the current store id, if there's no value, goes back to default store which is 0. 
