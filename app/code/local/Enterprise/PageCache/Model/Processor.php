...
class Enterprise_PageCache_Model_Processor {
    ...
    protected function _processContainers(&$content) {
        $placeholders = array();
        preg_match_all(
                Enterprise_PageCache_Model_Container_Placeholder::HTML_NAME_PATTERN, $content, $placeholders, PREG_PATTERN_ORDER
        );
    ...
    public function processRequestResponse(Zend_Controller_Request_Http $request, Zend_Controller_Response_Http $response
        ...
        if ($this->canProcessRequest($request)) {
            ...
            if ($processor && $processor->allowCache($request)) {
                ...
                $cacheInstance->save(
                        $currentStorageSize + $contentSize, self::CACHE_SIZE_KEY, $this->getRequestTags()
                );
                ...
                if ($designChange->getData()) {
                    $cacheInstance->save(
                            serialize($designChange->getData()), $this->getRequestCacheId() . self::DESIGN_CHANGE_CACHE_SUFFIX, $this->getRequestTags()
                    );
                }
    ...
    protected function _saveMetadata() {
        Enterprise_PageCache_Model_Cache::getCacheInstance()->save(
                serialize($this->_metaData), $this->getRequestCacheId() . self::METADATA_CACHE_SUFFIX, $this->getRequestTags()
        );
    }
