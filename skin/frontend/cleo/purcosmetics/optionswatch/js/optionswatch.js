...
jQuery( document ).ready(function() {
    ...
    mediaObject.reloadMedia = function(){
        ...
        var pdo_media_images = product_media_object.mediaImages;
            ...
            name = swatches[checkedId].option_value;
            jQuery('.swatch-sku').html('SKU: '+sku);
            jQuery('.swatch-name').html(' '+name);
            var image_array = pdo_media_images['product_'+sku];
            /** var option_select = document.getElementById('attribute296'); */
            /**
             *  WeltPixel customization - replace hard-coded attribute id above
             *  If there are more than one configurable attribute this piece of code need to be updated
             */
            var superAttrId = document.getElementById('super_attributes').value;
                superAttrId = superAttrId.split(',');
            var option_select = document.getElementById('attribute' + superAttrId[0]);
            /** End WeltPixel customization */
