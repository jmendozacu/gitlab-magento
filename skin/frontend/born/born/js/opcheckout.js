jQuery( document ).ready(function() {
    setTimeout(function(){ 
        var o = document.getElementById('billing:region_id-styler');
        jQuery(o).find('.jq-selectbox__select').remove();
        var p = document.getElementById('billing:country_id-styler');
        jQuery(p).find('.jq-selectbox__select').remove();
        jQuery(document.getElementById('billing:region_id')).attr('style', '');
        jQuery(document.getElementById('billing:country_id')).attr('style', '');
        jQuery(document.getElementById('shipping:region_id')).attr('style', '');
        jQuery(document.getElementById('shipping:country_id')).attr('style', '');   
        },
            1000
            ); 
});
...
Checkout.prototype = {
    ...
    setMethod: function(){
        if ($('login:guest') && $('login:guest').checked) {
            ...
            if(jQuery('.account-dropdown').length == 0){
...
//        $('billing:use_for_shipping_yes').checked = flag;
    ...
    syncWithBilling: function () {
        ...
        if (!$('billing-address-select') || !$('billing-address-select').value) {
            ...
            jQuery('#shipping\\:region_id').trigger('refresh');
...
Payment.prototype = {
    ...
    init : function () {
		...
		/* To enable PO field in checkout page */
		if ($('b2b_purchase_order_number')) {
             $('b2b_purchase_order_number').enable();
        }
        this.afterInit();
    },
