    ...
    frm.request({
        onComplete: function(trn){
            ...
            if(rsp.status != 'OK'){
                ...
                var newCardRow = new Template('<tr><td>#{cctype}</td><td>#{ccnumber}</td><td>#{exp}</td><td class="a-center"><div class="input-box"><input#{defaultchecked} id="default_rnk_#{id}" type="radio" value="#{id}" name="tokencard_def" onclick="tokenSetDefault(this);" /><label class="label-text" for="default_rnk_#{id}" onclick="tokenSetDefault($(\'default_rnk_#{id}\'));"></label></div></td><td class="last"><a href="#{delurl}" onclick="if(confirm(\''+Translator.translate('Are you sure?')+'\')){removeCard(this); return false; }else{return false;}">'+Translator.translate('Delete')+'</a></td></tr>');
